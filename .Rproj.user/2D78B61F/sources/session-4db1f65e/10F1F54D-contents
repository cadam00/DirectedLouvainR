...
usage(char * prog_name,
    const char * more) {
    Rcpp::Rcerr << more;
    Rcpp::Rcerr << "usage: " << prog_name << " input_file [options]" << endl << endl;
    Rcpp::Rcerr << "input_file: read the community tree from this file." << endl;
    Rcpp::Rcerr << "-l xx\t display the community structure for the level xx." << endl;
    Rcpp::Rcerr << "\t outputs the community for each node." << endl;
    Rcpp::Rcerr << "\t xx must belong to [-1,N] if N is the number of levels." << endl;
    Rcpp::Rcerr << "-n\t displays the number of levels and the size of each level." << endl;
    Rcpp::Rcerr << "\t equivalent to -l -1." << endl;
    Rcpp::Rcerr << "-h\tshow this usage message." << endl;
    Rcpp::stop(0);
}
...
main(int argc, char ** argv) {
    ...
    if (display_level == -1) {
        Rcpp::Rcout << "Number of levels: " << levels.size() << endl;
        for (unsigned int i = 0; i < levels.size(); i++)
            Rcpp::Rcout << "level " << i << ": " << levels[i].size() << " nodes" << endl;
        return EXIT_SUCCESS;
    }
    else if (display_level < -2) {
        Rcpp::Rcerr << "Incorrect level" << endl;
        return EXIT_FAILURE;
    } else {
        if (display_level == -2)
        ...
        if (display_level == 0) {
            ...
            for (unsigned int node = 0; node < levels[0].size(); node++)
                Rcpp::Rcout << corres[node] << " " << n2c[node] << endl;
