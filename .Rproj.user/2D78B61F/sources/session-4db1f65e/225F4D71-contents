...
Community::Community(const Graph& graph,
  ...
  this->community_graph = nullptr;  // safe delete in destructor
...
void Community::init_partition(string filename) {
  ...
  while (finput >> node >> comm) {
    ...
    for(i = 0 ; i < neighboring_communities; ++i) {//size; ++i) {
...
void Community::display() {
  for (unsigned int i = 0; i < size; ++i) {
    Rcpp::Rcout << " " << (this->g)->correspondance[i] << "/" << node_to_community[i] << "/"
...
void Community::display_partition() {
  ...
  Rcpp::Rcout << -1 << " " << -1 << endl;
  for (unsigned int i = 0; i < size; ++i)
    Rcpp::Rcout << (this->community_graph)->correspondance[i] << " " << renumber[this->node_to_community[i]] << endl;
}
...
bool Community::one_level(double &modularity) {
  ...
  if(randomized) {
    Rcpp::RNGScope scope; // tie into R's RNG
    std::shuffle(random_order.begin(), random_order.end(), std::default_random_engine(R::unif_rand() * UINT_MAX));
  }
...
void Community::print_level(int level) {
  ...
  for (unsigned int node = 0; node < this->g->nodes; node++)
    Rcpp::Rcout << (this->g)->correspondance[node] << " " << n2c[node] << endl;
}
...
int Community::run(bool verbose, const int& display_level, const string& filename_part) {
  ...
  do {
    if (verbose) {
      Rcpp::Rcerr << "level " << level << ":\n";
      Rcpp::Rcerr << "  network size: " <<
    ...
    if (verbose)
      Rcpp::Rcerr << "  modularity increased from " << mod << " to " << new_mod << endl;
...
#include "impl/community_friend_static.inc"
